...
// Relational synchronization logic has been moved to js/storage/relational.js


// Contest relational synchronization logic has been moved to js/storage/relational.js
...
async function nav(view, extra){
  ...
  if(view==='contest-detail' && S.currentContest) url.searchParams.set('contestId', S.currentContest);
  else url.searchParams.delete('contestId');
  
  if(window.location.search !== url.search) {
    history.pushState({view, extra}, '', url.href);
  }
  ...
  document.title = `BeSQL - ${titleMap[view] || 'SQL Contest Platform'}`;
...
window.addEventListener('popstate', (e) => {
  ...
  nav(view, contestId ? {contestId} : undefined);
});
